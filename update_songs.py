...
# Bảng tra cứu thông tin bài hát theo tên file nhạc
SONG_MAP = {
    # Khởi My
    ...
    "khoc_cung_em.m4a": ("Khóc Cùng Em", "Mr. Siro", "VPOP"),
    "khong_the_cung_nhau_suot_kiep.m4a": ("Không Thể Cùng Nhau Suốt Kiếp", "Mr. Siro", "VPOP"),
    "tu_lau_nuoc_mat.m4a": ("Tự Lau Nước Mắt", "Mr. Siro", "VPOP"),
    "co_don_tren_sofa.m4a": ("Cô Đơn Trên Sofa", "Mr. Siro", "VPOP"),
    ...
    "dem_trang.m4a": ("Đêm Trắng", "Lương Bích Hữu", "VPOP"),
    "co_le_nguoi_da_quen_toi.m4a": ("Có Lẽ Người Đã Quên Tôi", "Lương Bích Hữu", "VPOP"),
    "xem_nhu_em_chang_may.m4a": ("Xem Như Em Chẳng May", "Lương Bích Hữu", "VPOP"),
    "gap_lai_nhau_lam_gi.m4a": ("Gặp Lại Nhau Làm Gì", "Lương Bích Hữu", "VPOP"),
    "em_van_tin_vao_tinh_yeu_cua_anh.m4a": ("Em Vẫn Tin Vào Tình Yêu Của Anh", "Lương Bích Hữu", "VPOP"),
    "hinh_bong_cua_may.m4a": ("Hình Bóng Của Mây", "Lương Bích Hữu", "VPOP"),
    "anh_khac_hay_em_khac.m4a": ("Anh Khác Hay Em Khác", "Lương Bích Hữu", "VPOP"),
    "hon_ha.m4a": ("Hôn Hạ", "Lương Bích Hữu", "VPOP"),
    "nu_cuoi_moi.m4a": ("Nụ Cười Mới", "Lương Bích Hữu", "VPOP"),
    "tinh_yeu_ban_dau.m4a": ("Tình Yêu Ban Đầu", "Lương Bích Hữu", "VPOP"),
    "may_trang.m4a": ("Mây Trắng", "Lương Bích Hữu", "VPOP"),
    "loi_khuyen_cho_ban.m4a": ("Lời Khuyên Cho Bạn", "Lương Bích Hữu", "VPOP"),
    "yeu_lam_chi.m4a": ("Yêu Làm Chi", "Lương Bích Hữu", "VPOP"),

    # Quan Họ Bắc Ninh
    "beo_dat_may_troi.m4a": ("Bèo Dạt Mây Trôi", "Quan Họ Bắc Ninh", "QUAN HO"),
    "nguoi_oi_nguoi_o_dung_ve.m4a": ("Người Ôi Người Ở Đừng Về", "Quan Họ Bắc Ninh", "QUAN HO"),
    ...
    "muoi_nho.m4a": ("Mười Nhớ", "Quan Họ Bắc Ninh", "QUAN HO"),
    ...
    "lang_quen_chieu_thu.webm": ("Lãng Quên Chiều Thu", "Lương Thế Vinh", "VPOP"),
...
# Quét tất cả file audio thực tế
...
for idx, f in enumerate(audio_files, 1):
    if f in SONG_MAP:
        title, artist, genre = SONG_MAP[f]
    else:
        # Tự làm sạch tên file nếu chưa có trong danh sách
        clean_name = os.path.splitext(f)[0].replace("_", " ").title()
        title, artist, genre = clean_name, "Ca Sĩ Khác", "VPOP"
...
# Ghi đè file songs.json
with open(SONGS_JSON_PATH, "w", encoding="utf-8") as f:
    json.dump(new_songs, f, ensure_ascii=False, indent=2)

print(f"🎉 Hoàn tất! Đã cập nhật chuẩn xác {len(new_songs)} bài hát với đầy đủ Tên Ca Sĩ!")
